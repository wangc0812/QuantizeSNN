...
from QLayer.QConv2d import QConv2d
...
class BottleneckSNN(nn.Module):
    def __init__(self, inplanes, planes, downsample=None, stride2=False):
        ...
        self.layers = nn.Sequential(
            SNNLayer(QConv2d(inplanes, planes, kernel_size=1, bias=False)),
            SNNLayer(QConv2d(planes, planes, kernel_size=3, stride=2 if stride2 else 1, padding=1, bias=False)),
            # SNNLayer(QConv2d(planes, planes * expansion, kernel_size=1, bias=False))
        )
        self.residual_layer = SNNLayer(QConv2d(planes, planes * expansion, kernel_size=1, bias=False), bypass_in=True)
...
class ResNet2StageSNN(nn.Module):
    def __init__(self, firstchannels=64, channels=(64, 128), inchannel=3, block_num=(3, 4)):
        ...
        self.layers = nn.Sequential(
            SNNLayer(QConv2d(inchannel, firstchannels, kernel_size=7, stride=2, padding=1, bias=False)),
            ...
            *self._make_layer(channels[0] * expansion, channels[1], block_num[1], stride2=True),
            SNNLayer(QConv2d(channels[1] * expansion, channels[1] * expansion, kernel_size=1, bias=False),
    ...
    def _make_layer(self, inplanes, planes, blocks, stride2=False):
        ...
        downsample = QConv2d(inplanes, planes * expansion, kernel_size=3, stride=2 if stride2 else 1, padding=1, bias=False)
