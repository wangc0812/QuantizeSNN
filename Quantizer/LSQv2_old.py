...
class ALSQPlus(Function):
    ...
    @staticmethod
    def backward(ctx, grad_weight,grad_alpha,grad_beta):
...
batch of activations, respectively
        ...
        # else:
            # print(self.s, self.g)
        q_a,s_a,beta_a = ALSQPlus.apply(activation, self.s, self.g, self.Qn, self.Qp, self.beta)

            # alpha = grad_scale(self.s, g)
            # q_a = Round.apply((activation/alpha).clamp(Qn, Qp)) * alpha
        return q_a,s_a,beta_a
...
class LSQPlusWeightQuantizer(nn.Module):
    ...
    def forward(self, weight):
        ...
        w_q,w_a = WLSQPlus.apply(weight, self.s, self.g, self.Qn, self.Qp)
...
class LSQPlusuantizer(Quantizer):
    
    def __init__(self):
        super(LSQPlusuantizer, self).__init__()
        self.activation_quantizer = LSQPlusActivationQuantizer(a_bits=self.input_precision, Isigned=self.Isigned)
        self.weight_quantizer = LSQPlusWeightQuantizer(w_bits=self.weight_precision, Wsigned=self.Wsigned)
    ...
    def QuantizeInput(self, input, bits=None, Isigned=True):
        ...
        input,inputscale,inputshift = self.activation_quantizer(input) 
        inputrange = []   
